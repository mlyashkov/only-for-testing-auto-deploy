...
class GitAutoDeploy(BaseHTTPRequestHandler):
    ...
    def parseRequest(self):
        ...
        if body:
            ...
            items.append((item['repository']['name'], item['push']['changes'][0]['new']['name'])) #<------ (repoUrl, repoBranch)
        return items
    ...
    def getMatchingPaths(self, repoUrl, repoBranch):
        ...
        for repository in config['repositories']:
            if ((repository['url'].find(repoUrl)+1) and repository['branch'] == repoBranch):
    ...
    def pull(self, path):
        if(not self.quiet):
            ...
            print 'Updating ' + path
        g = git.cmd.Git(path)
        output = g.execute('git pull origin master')
        print output
